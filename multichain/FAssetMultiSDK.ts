...
export class FAssetMultiSDK {
  ...
  public async getAssetManagerAddress(asset: 'XRP' | 'BTC' | 'DOGE'): Promise<string> {
    ...
    try {
      ...
      if (address === '0x0000000000000000000000000000000000000000') {
        throw new Error(`AssetManager for ${asset} is not registered.`);
      }
      return address;
    } catch (err: any) {
      throw new Error(`Failed to resolve AssetManager for ${asset}: ${err.message}`);
    }
  }
  ...
  public async getSettings(asset: 'XRP' | 'BTC' | 'DOGE'): Promise<FAssetSettings> {
    ...
    // Direct mint settings query (ABI mapping properties for v1.3 settings)
  ...
  public async preparePayment(
    ...
    if (settings.paused) {
      throw new Error(`Direct minting for ${asset} is currently paused.`);
    }
    ...
    const decimalsMultiplier = asset === 'BTC' ? 100000000 : 1000000; // Satoshis (8 decimals) vs Drops (6 decimals)
